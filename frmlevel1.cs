    ...
    public partial class frmlevel1 : Form
    {
        ...
        private void frmlevel1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (ptrb1.Bounds.IntersectsWith(ptrb2.Bounds))
            {
                ...
                // Quay về trang chủ và refresh bảng điểm (không tạo form mới)
                var trangchu = Application.OpenForms.OfType<frmtrangchu>().FirstOrDefault();
                if (trangchu == null)
                    trangchu = new frmtrangchu();
                else
                    trangchu.RefreshBangDiem();   // Làm mới bảng điểm

                trangchu.Show();
                this.Close();   // Đóng form level1
                e.Handled = true;
                return;
            }
            e.Handled = true;
        ...
        private int CongDiem(int diem)
        {
            try
            {
                ...
                // Quan trọng: Dùng COALESCE để xử lý giá trị NULL trong cột DiemSo
